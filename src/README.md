...
```mermaid
graph TD
    A[grpc.go] --> B[setup.go]
    A --> C[proxy.go]
    A --> D[policy.go]
    C --> E[metrics.go]

    A -->|contains| F[GRPC struct]
    A -->|implements| G[ServeDNS]
    B -->|registers| H[Plugin]
    B -->|parses| I[Configuration]
    C -->|defines| J[Proxy struct]
    C -->|implements| K[gRPC client]
    D -->|defines| L[Policy interface]
    D -->|implements| M[Load balancing policies]
    E -->|defines| N[Prometheus metrics]
